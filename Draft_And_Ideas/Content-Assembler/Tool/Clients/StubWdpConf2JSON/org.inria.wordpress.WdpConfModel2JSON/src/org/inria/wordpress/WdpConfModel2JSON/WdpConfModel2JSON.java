...
public class WdpConfModel2JSON {
	...
	private String buildDeletion(Website webSite){
		...
		if (deletion != null){
			...
			EList<String> postTitles = deletion.getPostsByTitle();		
			String posts = "";
			for(String post : postTitles){	
				String keys [] = {"PostsByTitle"};
				String values [] = {post};
				List<List<String>> kVPost= WdpConfModel2JSONTools.getNotEmptyValuesAndKeys(keys, values);
				posts += WdpConfModel2JSONTools.toJsonArrayObject("Post", WdpConfModel2JSONTools.toJsonObjectList(kVPost.get(0), kVPost.get(1)));
				if (StringUtils.isNotBlank(posts) & WdpConfModel2JSONTools.isNotLastValue(post, postTitles.toArray())){
					posts += ",";
				}
			}
			posts = WdpConfModel2JSONTools.toJsonArrayObject("PostsByTitle",posts);
...
//			plugins = WdpConfModel2JSONTools.toJsonArrayObject("PluginsByFileName",plugins);
			...
			String[] content = {pages, posts, users, medias, functions, themes, categories, tags, menus, commentParams};
